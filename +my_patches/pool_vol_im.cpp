...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
 /*
  Implements max-pooling and returns the image in the format nr*nc*nd
 */
  ...
  if (nrhs < 3)
	 mexErrMsgTxt("Atleast 3 input arguments required. patches = pool_vol_im(im,patchSz,stepSz,nThread)");
 ...
 for (int i=0; i<cNum; i++){
  ...
  plhs[1] = mxCreateNumericMatrix(2*nd,numPatches,mxUINT32_CLASS,mxREAL);

  pool    = (float *) mxGetPr(plhs[0]); //Store the max-pooled value
  maxLoc  = (uint32 *) mxGetPr(plhs[1]); //Store the location of top-left corner of the patch which produced the maxValue. 
  ...
  for (int c=0; c<cNum; c++){
	for (int r=0; r<rNum; r++){
		...
		for (int d=0; d<nd; d++){
			...
			//mexPrintf(" \n");
			//mexPrintf("Locs: %d,%d \n",mLoc[0],mLoc[1]);
			pool[cNum*rNum*d + c*rNum + r]     = maxVal;
